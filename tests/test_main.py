...
from sqlalchemy.orm import (
    sessionmaker,  # sessionmaker is used to create a new session for interacting with the database
)

from app.main import app, get_db  # Importing the FastAPI app and the get_db dependency
from app.setup_database.models import AirPollutionData, Base  # Importing the database models
...
def test_get_stats_valid_year_range():
    response = client.post(
        "/get_stats/", data={"entity": "Albania", "start_year": 2000, "end_year": 2020}
    )  # Simulate a POST request with a valid year range
    assert response.status_code == 200  # Check that the response status code is 200 (OK)
...
def test_get_stats_invalid_year_range():
    response = client.post(
        "/get_stats/", data={"entity": "Albania", "start_year": 1700, "end_year": 2025}
    )  # Simulate a POST request with an invalid year range
    assert response.status_code == 200  # Check that the response status code is 200 (OK)
    assert (
        "<p>Year range must be between 1750 and 2022</p>" in response.text
    )  # Check that the response contains the expected error message


def test_get_stats_exception_handling():
    response = client.post(
        "/get_stats/", data={"entity": "Albania", "start_year": "invalid", "end_year": "invalid"}
    )  # Simulate a POST request with invalid year values
...
def test_get_stats_valid_data():
    ...
    response = client.post(
        "/get_stats/", data={"entity": "MyCountry", "start_year": 2000, "end_year": 2002}
